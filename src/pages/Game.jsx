...
const Game = ({ session }) => {
  ...
  <div className="flex flex-col h-[100dvh] max-w-md mx-auto p-4 bg-slate-950 text-white relative overflow-hidden">
    {renderNotification()}
    ...
    <h2 className="text-[10px] font-black text-slate-400 text-center mb-1 uppercase tracking-[0.2em] px-2 shrink-0">
      {currentLevel?.pregunta}
    </h2>

    {/* Contenedor de imagen: Le bajamos un poco el mb (margen inferior) de 4 a 2 para ganar espacio */}
    <div className={`relative flex-1 min-h-0 w-full rounded-[2rem] border-4 flex items-center justify-center mb-2 overflow-hidden transition-all shadow-2xl ${
    ...
    {/* Ayudas: mb de 4 a 2 */}
    <div className="flex gap-3 w-full mb-2 shrink-0">
      <button onClick={revealHint} disabled={coins < 30 || status !== 'playing'} className="flex-1 bg-slate-900 border border-slate-800 p-2 rounded-2xl flex flex-col items-center disabled:opacity-30 active:scale-95 transition-all group shadow-md">
        <span className="text-[8px] text-slate-500 group-hover:text-white uppercase font-black tracking-widest mb-1 transition-colors">Revelar</span>
      ...
      <button onClick={skipLevel} disabled={coins < 80 || status !== 'playing'} className="flex-1 bg-slate-900 border border-slate-800 p-2 rounded-2xl flex flex-col items-center disabled:opacity-30 active:scale-95 transition-all group shadow-md">
        <span className="text-[8px] text-slate-500 group-hover:text-white uppercase font-black tracking-widest mb-1 transition-colors">Saltar</span>
        <span className="text-amber-500 font-bold text-xs">💰 80</span>
      </button>
    </div>

    {/* Espacio de respuesta: Reducimos un poco el alto (w-7 h-9) y el mb de 4 a 3 */}
    <div className="flex gap-1 mb-3 flex-wrap justify-center min-h-[36px] shrink-0">
      {selectedLetters.map((slot, i) => (
        <div key={i} onClick={() => handleRemove(i)} className={`w-7 h-9 flex items-center justify-center text-base font-black rounded-t-lg border-b-4 transition-all cursor-pointer shadow-inner ${
    ...
    {/* Teclado: Gap y padding más pequeños (gap-1 p-1) */}
    <div className="grid grid-cols-6 gap-1 mb-2 p-1.5 bg-slate-900 rounded-2xl border border-slate-800 shadow-inner shrink-0">
      {availableLetters.map((l) => (
        <button key={l.id} disabled={l.used || status === 'success'} onClick={() => handleSelect(l)}
          className={`aspect-square rounded-xl font-black text-base transition-all shadow-md ${
            l.used ? 'bg-slate-900 text-slate-800 scale-95 opacity-50' : 'bg-white text-slate-900 active:scale-75 shadow-[0_2px_0_0_#cbd5e1]'
          }`}>
          {l.char}
        </button>
      ))}
